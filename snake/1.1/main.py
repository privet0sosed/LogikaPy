def RunGame(cells=14, CELL_SIZE=32, bgimg=None):
    ...
    while running:
        ...
        if not melon.eaten and frame == 0 and score > 8 and random.randint(0, 45):
            melon.respawn()

        if not gapple.eaten and frame == 0 and score > 16 and random.randint(0, 75):
            gapple.respawn()
        ...
        if snake.slow>0 and frame%30 == 0:
        ...
        if math.floor(tick()%50) == 0:
            melon.eaten = False
        if math.floor(tick()%90) == 0:
            gapple.eaten = False
